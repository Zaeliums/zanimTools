...
def create_lip_nodes(naming_convention, is_mirror_behavior):
    ...
    if is_mirror_behavior:
        ...
        stripped_names = [ctrl.replace(naming_convention.side_l, "").replace(naming_convention.side_r, "") for ctrl in
                          controlList]
    ...
    def create_nodes_for_selected(control, is_mirror_behavior):
        ...
        cmds.connectAttr(naming_convention.jaw_joint + ".rotate", control + "_multi.input1",
                         f=1)  # Connect Jaw rotate with the multiply node (to allow lip rotation to match jaw rotation)
        cmds.connectAttr(naming_convention.jaw_control + ".StickyLips", control + "_remap_pressed.inputValue",
                         f=1)  # Connect Jaw controller attribute "Sticky Lips" to the input value of the _remap_pressed node

        if naming_convention.pos_bot_name in control:
            cmds.connectAttr(control + "_remap_pressed.outValue", control + "_inv.inputValue",
                             f=1)  # Conditionally connect the _remap_pressed.outValue
            cmds.connectAttr(control + "_remap.outValue", control + "_remap_pressed.outputMax",
                             f=1)  # Conditionnally connect _remap outValue to the remap_pressed outputMax
        ...
        cmds.connectAttr(naming_convention.jaw_control + ".StickyTopBot", control + "_remap.inputValue",
                         f=1)  # Connect Jaw controller attribute "Sticky Top Bot" to the input value of the _remap node
        cmds.connectAttr(control + "_multi.output", control + "_plus.input3D[0]",
                         f=1)  # Connect _multi outputs to the correct nodes and into the _drivers
        ...
        cmds.connectAttr(naming_convention.jaw_control + ".PressLips", control + "_plus.input3D[1].input3Dz",
                         f=1)  # Connect "Press Lips" attribute with the add node and then this node to the controller_driver
        ...
        if is_mirror_behavior:
            ...
            if mirrored_control and cmds.objExists(mirrored_control + "_driver"):
                ...
                if control.startswith(naming_convention.side_l or naming_convention.side_r):
                    print("Mirrored control could not be set up for: {}. Check naming conventions or existence.".format(
                        control))
